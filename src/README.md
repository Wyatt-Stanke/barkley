...
For a modernized migration (one whose `key_doset` calls `pad_keys`) it also adds `gamepad.js` (`extensions/Gamepad/`): `pad_keys(up, down, left, right, action, cancel, start)` records the bound keys and `pad_context(n)` the same context the touch overlay gets. It polls the Gamepad API once a frame and sends those keys through the runtime's `window.onkeydown` / `window.onkeyup`, as `touch.js` does, so a pad press is a key press everywhere in the game.
