...
For a modernized migration (one with `scripts/sControls.gml`) it also adds `controls.js` (`extensions/Controls/`): `controls_open(up, down, left, right, action, cancel, start)` shows the Controls panel the title screen's fifth row opens.
...
| `web/controls.js` | The HTML5 extension `import.mjs` adds to a modernized migration: the Controls panel, with the live input test. |
...
4. Apply the patches:
   ...
   - Then, in modernized mode only (the default), `patches/modernized/*.patch`:
     ...
     - `12-controls-panel`: a fifth row on the title menu, Controls, which opens a panel on the page (`controls.js`) through the new script `sControls`. The panel lists the seven controls with the keys they are bound to right now (and the W A S D / J K aliases where they still stand), says how to rebind them (Configuration → SET KEYS, and SETTINGS → Default to undo), what a controller does, and then tests it: every control lights as it is pressed from either the keyboard or a pad, with the pad's name, its raw buttons and its sticks beside them, and the last key pressed named as the control it works or as bound to nothing. Nothing pressed in the panel reaches the game: key events stop there and `pad_quiet` mutes the pad while it is open. `sTitle0` holds only four rows, so the Draw event draws it without its bottom frame, extends it by a row from the sprite's own blank interior, and spells `Controls` out of letters cut from the other three labels — the sprite's lettering is heavier than any font in the game, and every letter of the word is already in it.
