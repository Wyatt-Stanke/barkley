...
- Node.js 24+ (no npm packages; `playtest.mjs` uses the built-in `WebSocket`, the scripts `import.meta.main`). The page around the game, `web/`, is the exception: a Vite + SolidJS + TypeScript project whose packages `page.mjs` installs from `web/package-lock.json` with `npm ci` the first time it builds. Peggy is only needed to regenerate the parser after editing the grammar: `npx -y peggy@5.1.0 --format es --allowed-start-rules Program,Tokens -o src/lib/gml.parser.mjs src/lib/gml.peggy`
...
The importer skips the HTML5 options, so `import.mjs` writes `options/html5/options_html5.yy` itself. It sets the page title to the game's name, names `barkley_loading` (the page's) as the loading bar, and uses `web/index.html`, copied beside the options, as the custom index. Igor finds the index only by absolute path, so the project records where it was imported.

It also adds the GameMaker extensions that declare the functions the GML calls into the page: `Fullscreen` always (`fullscreen_set`, `fullscreen_get`, which patches 14 and `modernized/05` call, since the runtime's `window_set_fullscreen` does nothing on HTML5), and for a modernized migration `Resume` (`resume_put`, `resume_take`, `resume_clear`), `Saves` (`saves_open`), `Touch` (`touch_keys`, `touch_context`, `touch_active`, `touch_view_*`, `touch_dpr`), `Gamepad` (`pad_keys`, `pad_context`), `Controls` (`controls_show`, which no GML calls) and `Crash` (`crash_put`, `crash_step`, `crash_wanted`, `crash_end`), each gated on a script the modernized patches add. The page defines every one of these functions; an extension's JavaScript file is a one-line stub, `barkley.extension("Touch");`, which the runtime loads with the game and which turns that part of the page on. So a faithful migration, which has only `Fullscreen`, gets none of the rest.

### The page (`web/`)

Everything around the game in the browser is `web/`, a Vite + SolidJS + TypeScript project. `page.mjs` builds it (`npm run build`: `tsc --noEmit`, then `vite build`) into `build/web/`, and `offline.mjs` copies that over every HTML5 build, so a change to the page needs no re-import:

- `web/index.html`: the page template Igor builds with, which is the runtime's own with its placeholders. It loads `app/barkley.css`, the font and the runtime's own styles, then `app/barkley.js`, a classic script placed **before the game's script**: the runtime captures `requestAnimationFrame` as its script runs, so the page's hooks have to be in first, and a module script would run after it. Last, `window.onload = barkley.load`, where the runtime's template has `window.onload = GameMaker_Init`.
- `web/src/`: the app, one IIFE bundle (strict and unminified, ~116 KB): `main.tsx` (the entry and `window.barkley`), `page.ts` (shared state), `start.ts` (loading, the hold and Start), `runtime/` (the hooks into the runtime), `offline.ts`, `install.ts`, `extensions/` (one module per GameMaker extension, with the functions the GML calls) and `ui/` (the Solid components, each with its style sheet).
- `web/public/`: copied into the build as it is: `sw.js` at its root, and the font, Inter (Latin subset, variable weight and optical size, from `@fontsource-variable/inter` 5.3.0), with its SIL Open Font License in `app/`.
- `web/pwa/`: the app manifest and the icon, which `import.mjs` adds to the project as Included Files (they land in `html5game/`).

What the page does:
- the design system: every piece of UI around the game (Start, the install prompt, the Controls, Saves and crash panels, the touch overlay and its sheet). Swiss and deliberately forgettable: black, white and a muted white, Inter (Helvetica/Arial as fallbacks), flush left on a 16 px gutter (32 px from 720 px wide), hairline rules, square 44 px buttons, and an active control inverts to white. Every panel is one component (`ui/Panel.tsx`): a dialog above the touch overlay's hit layer, focused when it opens, with Close and Esc where it can be closed, that keeps the keys pressed in it from the game but lets through the release of a key pressed before it opened;
- a Start screen that is itself the Start button (the whole page): the game's name, "Start" in large type and a hint, set on the left edge of where the 4:3 picture will be. The game loads behind it (`GameMaker_Init` runs on page load, and the runtime's own loading bar is replaced by the word itself, which fills from grey to white as the game loads, by bytes: the texture pages download through `fetch` so their progress can be read) and is held just before Game Start (a `console.log` hook throws out of the runtime's frame at "Entering main loop...", and a `requestAnimationFrame` wrapper keeps every frame after it back); a click, a tap, or any character key, Enter or an arrow then starts it at once, which also lets audio start after a user gesture. When a resume state is waiting (`localStorage` `barkley.resume`, so modernized builds only), a line at the foot of the page says the game continues where it left off and offers "Start from the title screen": clicking it (or Enter or Space on it) deletes the state before Game Start's `resume_take` reads it, so the game starts from the intro. Under the fuzz harness it starts the old way (Start runs `GameMaker_Init`), and the page starts no frame loops of its own;
- a Controls link in the bottom corner of the Start screen, shown once the game has loaded when the build has the `Controls` extension, which opens the Controls panel: the seven controls with the keys bound to them (read from the game's own `controls.txt` in browser storage, so they are the player's, not the defaults) and the W A S D / J K spares, where to change them once the game is running, what a controller does, and a test that lights each control as it is pressed from the keyboard or a pad and names a connected pad with its raw buttons and sticks. It is deliberately before the game: a player who cannot work out how to drive it is standing here. Nothing pressed in it starts the game — the panel keeps its key events, the Start screen's key handler stands down while it is up, and the pad is muted — and it polls on a timer, because the Start screen holds frame callbacks back until the game starts. Closing it puts focus back on Start;
...
- links that make the page installable as an app (PWA) that opens full screen: `manifest.webmanifest` (`display: fullscreen`, falling back to `standalone`; black theme and background; no orientation lock, since the touch overlay lays out both ways), an `apple-touch-icon`, `theme-color` and `mobile-web-app-capable`. `import.mjs` adds the manifest and `icon-180/192/512.png` (resized with ffmpeg from `web/pwa/icon.png`) to the project as Included Files, which the build copies into `html5game/`; the manifest's `start_url` and `scope` are `../`, the page itself. The service worker that makes it play offline ships from the build root instead ("Offline play" below). To change the icon, replace `web/pwa/icon.png` (square) and import again.
...
And what the extensions do, once their stubs have turned them on:
- `Fullscreen` (`extensions/fullscreen.ts`): `fullscreen_set(on)` and `fullscreen_get()` fullscreen the whole page. Browsers allow entering fullscreen only shortly after a key press or click; without one, `fullscreen_set(1)` waits for the next key press, click or tap.
- `Resume` (`extensions/resume.ts`): `resume_put(state)` keeps the game state patch `modernized/06` hands it and writes it to `localStorage` (key `barkley.resume`) when the page is hidden or closed, and at most every 5 s; `resume_take()` returns it once per page load and deletes it; `resume_clear()` forgets it.
- `Saves` (`extensions/saves.ts`, `ui/SavesPanel.tsx`): `saves_open(probe)` shows the save-transfer panel (Export and Import columns; Esc or Close shuts it). `probe` is the name of a file the game has just written, which is how the page finds the prefix the runtime puts on its storage keys.
- `Touch` (`extensions/touch.ts`, `ui/TouchOverlay.tsx`): the touch overlay (patch `modernized/10`).
- `Gamepad` (`extensions/gamepad.ts`): `pad_keys(up, down, left, right, action, cancel, start)` records the bound keys and `pad_context(n)` the same context the touch overlay gets. It polls the Gamepad API once a frame and sends those keys through the runtime's `window.onkeydown` / `window.onkeyup`, as the touch overlay does, so a pad press is a key press everywhere in the game. The Controls panel mutes it to read the pad without the game hearing it.
- `Controls` (`extensions/controls.ts`, `ui/ControlsPanel.tsx`): the Controls panel. It reads the player's own key bindings out of the game's `controls.txt` in browser storage (`key_save` writes it) rather than being told them, because the game has not run yet.
- `Crash` (`extensions/crash.ts`, `ui/CrashPanel.tsx`): the crash recorder and report (patch `modernized/07`).
...
## Offline play (`src/offline.mjs`, `src/web/public/sw.js`)
...
- **The page** (`web/src/offline.ts`) registers the worker only once the game has loaded, so it never competes
  ...
  server has, and how it serves a copy it saved before. `isApp()` (a `display-mode` of
  ...
  The version alone (`v1.0.0`) is shown by `loadVersion()`, which reads the build's own `version.json`
...
Paths are relative to `src/`. The `web/` folder holds everything that ships into the built page ("The page"
above): `import.mjs` copies the template and the PWA files into the project, and `page.mjs` builds the rest into
every HTML5 build.
...
| `web/index.html` | The HTML5 page template the build uses: the metas and app manifest links, the font, and the page app loaded before the game. |
| `web/src/` | The page app (SolidJS, TypeScript): the Start screen, the install prompt, the Controls, Saves and crash panels, the touch overlay, the hooks into the runtime (the hold before Game Start, the loading progress, the audio fixes), offline play, and the functions the GML calls through the extensions. |
| `web/public/` | Copied into every build as it is: `sw.js`, the service worker, and `app/inter.woff2` with `app/inter-OFL.txt`, the page's font, Inter (Latin subset, variable weight and optical size, from `@fontsource-variable/inter` 5.3.0), and its SIL Open Font License. |
| `web/pwa/` | The web app manifest and the source of its icons, added to the project by `import.mjs` so the page installs as a full-screen app. |
...
4. Apply the patches:
   ...
   - `14-fullscreen`: `oController`'s Game Start calls `fullscreen_set` (the `Fullscreen` extension's) in place of `window_set_fullscreen`, so the Configuration menu's `SCREEN Windowed / Full Screen` works again. As in the original, it applies when Settings → Exit restarts the game, and at startup (from the first key press or click).
   ...
   - Then, in modernized mode only (the default), `patches/modernized/*.patch`:
     ...
     - `09-save-transfer`: on the Configuration menu's SETTINGS row, Saves replaces Test. (Test opened `RomTest`, a view-scaling check that means nothing now that `oScreenFill` presents the game and the SCALING row picks Integer or Sharp fit. A fourth option does not fit: the blip's pitch is fixed, so with four labels a blip either overlaps the previous label or falls outside the panel.) It calls `sSaveData`, which opens the browser save transfer (the `Saves` extension's panel): a panel holding the three save slots as one block of Base64 gzip text, with Copy and Download, and a box to paste one back. Saves live in browser storage, which is per origin, so they are lost by clearing site data or switching browser; this carries them. Only `Save1..3.sav` travel, never the settings or the `SaveN0.sav` temp file. `sSaveData` writes a probe file and passes its name, because the runtime's storage keys carry a prefix built from the game's name and id (`BarkleyShutUpandJamGaiden.0.`) that the game cannot read; the key ending with the probe gives it. `file_exists` and `file_text_open_read` read storage live, so an imported slot shows up in Load Datafile without a reload.
     - `10-touch-controls`: the mobile touch overlay (the `Touch` extension) and a canvas backed by device pixels. `key_doset` passes the bound keys to `touch_keys`, `oController`'s Begin Step passes `sTouchContext()` to `touch_context`, and `oScreenFill` sizes the canvas with `touch_dpr()` and draws into `touch_view_*()`.
     - `11-gamepad`: game controllers (the `Gamepad` extension). `key_doset` passes the bound keys to `pad_keys` and `oController`'s Begin Step passes `sTouchContext()` to `pad_context`, so a pad follows SET KEYS and sends nothing while the player is rebinding. The page polls the Gamepad API each frame and sends those keys: the d-pad and left stick steer (a direction turns on past 0.45 of the stick's travel and off again inside 0.3, and repeats while held, as an arrow key does), the bottom and left faces and both shoulders confirm or run, the right and top faces cancel, Start and Back open the menu. A press is held out to 100 ms so a tap between two of the game's 30 fps steps is never lost. The game's own GM6 joystick code (`key_joyemu`) is untouched: its one call site was already commented out in the original.
