...
//
// writeBuild(dir) puts web/sw.js at the root of an HTML5 build — a service worker only controls pages under its own
// path, so it can't ship from html5game/ as the other page files do — and writes version.json beside it: the project's
// version (src/version.json, semver), an id that is the hash of the whole file list, and every file the page can ask
// for with its hash, its size, and whether the game needs it before the first frame. The id is what the worker
// compares to decide a build is new; the hashes are what lets it keep the files a new build didn't change.
...
// index.html reports no Ogg support so the runtime loads the MP3 of every sound (Safari's Vorbis decoder clips them),
...
export function writeBuild(dir) {
  copyFileSync(path.join(import.meta.dirname, 'web', 'sw.js'), path.join(dir, 'sw.js'));
  const files = [];
  (function walk(rel) {
