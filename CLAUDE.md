...
node src/fuzz.mjs build build/outputs/barkley-1.1.0/BarkleyLTS.yyp build/fuzz/build
...
- `build/outputs/`: migrations and their imports. **The user wants only the latest kept**: when you make a new one, delete the superseded migration and import (the user OK'd that).
  - Latest: `barkley-1.1.0.gmx` (plus `.code`) and its import `barkley-1.1.0/BarkleyLTS.yyp` (2026-09-20): audit clean, importer converted all GML, all five extensions plus the PWA files, `extensions/` identical to the previous import's, and the app icons regenerated from the new `src/web/icon.png`. This is what is deployed. The offline layer needs no re-import: `fuzz.mjs build` writes `sw.js` and `version.json` into the build.
...
**Live: `58167a6`** (2026-09-20), v1.1.0, the modernized build of `barkley-1.1.0`, deployed with `deploy.mjs`: offline play, the rebuilt battle-start transition, the dialog key fix and the new app icon. Build id `70ecdf979f2757f2`, 260 files cached. `BarkleyLTS.js` md5 `a0cd41a4e29b8a5bf66fa6caf155b3f1`; `index.html` md5 `10d322fdf9cf1b6a68233204d5c00513`; `sw.js` md5 `8352242ca58c04a4c488f81a8d89eef7`; `version.json` md5 `e9456ad95509cb9b426b4a92bfd305ff`; shims `tph_crash.js`, `uph_touch.js`, `vph_saves.js`, `wph_resume.js`, `xph_fullscreen.js`, plus `inter.woff2` and `inter-OFL.txt`. Earlier deploys: `46e21c9` (patch 19, battles drawing blank), `db2beaf` (a rebuild of the same v29 code, no change), `3728aa7` (the iOS touch fixes and the streamed-music guard), `224a51a` (the Start screen's option not to resume), `479ed0f` (v32, redesigned page, loading behind Start), `36ef5ee` (v28, installable app), `9e360b0` (v27, touch controls), `be2ba4e` (v26, volume, Saves panel), `83e6d2b` (v25, first minified unobfuscated deploy), and older ones back to v12.
...
- **v1.1.0 is deployed (`58167a6`, 2026-09-20) and is the current state of everything.** It merges four strands of parallel work that had been sitting on separate branches: the offline layer, the new app icon, the rebuilt battle-start transition and the dialog key fix. `barkley-1.1.0` is the migration and import behind it. Checked on that build before deploying: the audit is clean, the importer converted all GML, a forced `oBDreadref` battle plays its transition (a cell pattern over the battle, which runs underneath from the first frame as the patch documents) and then draws the backdrop, battler, shadow and both HUD views, and the dialog key is consumed once per press; `deploy.mjs`'s own play-test and its live md5 checks passed.
