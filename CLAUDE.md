...
node src/fuzz.mjs build build/outputs/barkley-1.0.0/BarkleyLTS.yyp build/fuzz/build
...
barkley/
  ...
  src/           all the tooling (Node.js 22+, no npm packages)
    *.mjs        the pipeline: migrate, import, playtest, fuzz, deploy, assets, transforms, offline
    ...
    web/         what ships into the page: index.html, the 5 extension shims, sw.js, the PWA assets
...
- `build/outputs/`: migrations and their imports. **The user wants only the latest kept**: when you make a new one, delete the superseded migration and import (the user OK'd that).
  - Latest: `barkley-1.0.0.gmx` (plus `.code`; modernized, audit clean, byte-identical across two runs) and its import `barkley-1.0.0/BarkleyLTS.yyp`: importer converted all GML, all five extensions plus the PWA files. These were `BarkleyMigrated30.gmx` and `BarkleyLTS35` before the move to semver (2026-09-20); nothing in them changed, and the stale absolute `option_html5_index` was repointed at the new path. The offline layer needs no re-import: `fuzz.mjs build` writes `sw.js` and `version.json` into the build.
