...
- `build/outputs/`: migrations and their imports. **The user wants only the latest kept**: when you make a new one, delete the superseded migration and import (the user OK'd that).
  - Latest: `barkley-1.0.1.gmx` (plus `.code`) and its import `barkley-1.0.1/BarkleyLTS.yyp` (2026-09-20): audit clean, importer converted all GML, all five extensions plus the PWA files. **It predates this merge** — it has the dialog key fix but not the new icon, the offline layer or the battle-start transition, so it must be regenerated before the next deploy. The offline layer itself needs no re-import: `fuzz.mjs build` writes `sw.js` and `version.json` into the build.
...
- **Four changes are merged and none of them is deployed** (2026-09-20): the offline layer, the new app icon, the battle-start transition and the dialog key fix. The live deploy is still `46e21c9`, which predates all of them. The migration and import on disk are older than the merge, so regenerate them, then deploy.
