...
#    script the modernized patches add; each extension's file is a one-line stub that turns its part of the page on),
#    and Included Files for the page (src/web/pwa/manifest.webmanifest, icon-180/192/512.png resized with ffmpeg from
#    src/web/pwa/icon.png; to change the app icon, replace icon.png and re-import).
...
#    in a js: step (the page polls it every frame, so changing the fake pad's buttons or axes drives the game).
...
node src/page.mjs
cd src/web && npm run check   # type-check only
...
barkley/
  ...
  src/           all the tooling (Node.js 22+, no npm packages but the page's)
    *.mjs        the pipeline: pipeline (all of it), fetch, toolchain, migrate, import, playtest, fuzz, deploy, assets, transforms, offline, page
    ...
    web/         the page around the game (Vite + SolidJS + TypeScript): index.html (Igor's template), src/ (the app:
                 the Start screen, panels, touch overlay, and the functions the GML calls), public/ (sw.js, the font),
                 pwa/ (manifest and icon, Included Files); node_modules/ is untracked
...
- **`fuzz.mjs` and `fuzz-page.js`**: the fuzzer; `src/README.md` ("Fuzzing") describes the design (virtual time, snapshots, Go-Explore search, generators, crash verification). Facts not in README:
  ...
  - More game facts they rely on: a cutscene's quick-time event is an `oQuicker` (press the key its `key` 0-5 shows,
    right/up/left/down/action/cancel, within ~20 frames; a miss costs `global.die`), and the plot-3 cutscene in NeoYork1
    that leads to the catacombs is a run of them; a dialog with a choice has `option[0]!='0'` and its cursor in `cho`;
    entering a room sets `global.freeze` for ~20 frames; `global.battlers` counts the monsters roaming a map room, so
    `probe.battle` is not a fight (it stays 1 in a fight begun in the catacombs and is 0 in a boss fight a cutscene
...
- **The page (`src/web/`)** is a Vite + SolidJS + TypeScript project, the one part of the port with npm packages (`src/page.mjs` runs `npm ci` when `node_modules` is missing or older than the lockfile, then `npm run build`: `tsc --noEmit`, then `vite build` into `build/web/`). It builds **one classic IIFE script, `app/barkley.js`, plus `app/barkley.css`**, strict (Rolldown drops `"use strict"` unless `output.strict: true`) and unminified (~116 KB, ~31 KB gzipped; readable in a deployed build). `public/` is copied as it is: `sw.js` to the root and the font to `app/`. `offline.mjs`'s `writeBuild` copies `build/web/` over every HTML5 build before writing `version.json`, so **a page change never needs a re-import**. Layout:
  ...
  - `src/page.ts`: shared state (`ready`, `started` signals, the extension registry, `isApp()`, a `localStorage` wrapper that never throws). `src/start.ts`: `markReady`, `start`, `load` (window.onload) and the Start screen's key handler. `src/runtime/`: `hold.ts` (the hold before Game Start), `loading.ts` (`barkley_loading` and the texture byte progress), `audio.ts` (no Ogg, the AudioContext record, `earlyMusic`, `fixStreamedSounds`), `keys.ts` (`sendKey`: a synthetic key through `window.onkeydown`/`onkeyup`). `src/offline.ts`, `src/install.ts`.
  - `src/extensions/`: one module per GameMaker extension, with the functions the GML calls (`fullscreen_*`, `resume_*`, `saves_open`, `touch_*`, `pad_*`, `controls_show`, `crash_*`) and each one's state as Solid signals and stores. `src/ui/`: the components, each with its CSS beside it (`base.css` holds the tokens and buttons and is imported first).
  - **The extensions are stubs.** `import.mjs` writes each extension's file as one line, `barkley.extension("Touch");`, with its function declarations in the `.yy`. The GML's functions are page globals whatever the build, but what an extension does on its own (the crash recorder's listeners, the touch overlay, the pad poll, the resume listeners, the Controls link) starts only when its stub runs, so a faithful build, which has only Fullscreen, gets none of it. The stubs run as the runtime loads them, during `GameMaker_Init` (after `load` registered the Start screen's key handler, so the crash recorder never sees a key the Start screen kept from the game).
  ...
  - **Font: Inter** (SIL OFL 1.1; the Latin subset with the weight and optical-size axes, from `@fontsource-variable/inter` 5.3.0), self-hosted as `app/inter.woff2` with `inter-OFL.txt` beside it (`src/web/public/app/`), preloaded, `font-display: swap`, falling back to Helvetica/Arial. The code boxes use the system monospace.
  ...
  - **The design** (Swiss, monochrome, Inter, flush left; the user wants it forgettable, the game the centrepiece) lives in the components' style sheets (`.ui-panel`/`.ui-head`/`.ui-cols`/`.ui-code`/`.ui-btn`/`.ui-link`, `#gmtouch`/`.gmt-*`, `.ctl-*`). Every panel is `ui/Panel.tsx`: a `role="dialog"` sheet above the touch overlay's hit layer (`z-index` 2147483600 vs 2147483000), focused when it opens, with a Close link and Esc when it has `onClose`. It keeps the key events pressed inside it (native `on:keydown`, stopped before they reach the runtime's `window.onkeydown`), but passes through the release of a key pressed before it opened.
  - A `<button id="start">` covering the whole page, with its words (the game's name, "Start" in large type, a hint, the version line) drawn over it by `#face`, which has `pointer-events: none` so every click but the offline link's reaches the button (a button can't hold a link). **The game loads behind it:** `load` calls `GameMaker_Init()`; `import.mjs` sets `option_html5_loadingbar` to `barkley_loading`, which the runtime calls every loading frame instead of drawing its bar, but only once the extension scripts have loaded (so a 100 ms timer also runs `showProgress`). The runtime's frame state machine starts the game in the same frame that sees the last asset loaded, so the page holds it at the one gap: a `console.log` hook throws a sentinel at "Entering main loop..." (logged after all loading, just before Game Start), a `requestAnimationFrame` wrapper catches it and keeps later frames back, and Start is enabled and focused. A click, a tap, or a character key, Enter or an arrow (`barkley.start()`) removes the Start screen, resumes the recorded AudioContexts (the runtime unlocks audio on pointer presses only) and releases the frames, so the game starts at once. Keys before Start never reach the runtime. **Not resuming:** when `localStorage` holds `barkley.resume`, `#fresh` (a footer line flush with the words: "Continues where you left off." and the link `#fresh-go`, "Start from the title screen") shows; it's enabled with Start, and it or Enter/Space on it calls `barkley.start(true)`, which removes the key before Game Start's `resume_take` reads it. **Under the fuzz harness (`window.__fuzz`) none of this applies:** Start is enabled on load and runs `GameMaker_Init()`, because the harness wraps the game functions before init and runs the frames itself; the page also starts no frame loops of its own (`touch_pin`, `pad_poll`). `playtest.mjs` and `fuzz.mjs` find Start by id. `playtest.mjs` clicks it at once, so to screenshot the Start screen or the install prompt, use a scratch copy with the Start-click loop skipped.
  - **The foot of the Start screen** (`#foot`, bottom left, flush with the words) stacks its rows, however they wrap: `#ctlbar` (the Controls link) above `#fresh`.
  - **The Controls link** (`#ctlbar`/`#controls-go`): shown once the game has loaded, when the Controls extension is in the build, and `barkley.controls()` opens `ControlsPanel`. It lists the seven controls with the keys bound to them, read from the game's `controls.txt` in browser storage (the game has not run, so nothing can tell the page), the W A S D / J K spares, where to rebind, what a pad does, and a live test of all of it. **Nothing pressed in it may start the game:** the panel keeps its own key events, the Start screen's `keydown` handler returns while it is open (Enter or Space on the link opens it instead), and `padQuiet` mutes the gamepad extension. **It polls on a `setInterval`, not `requestAnimationFrame`:** before Start the page's rAF wrapper holds every callback back, which froze the test where it is most used. Closing it puts focus back on Start; Start closes it.
  - A hard-coded black page background (Igor substitutes `${GM_HTML5_BackgroundColour}` as the invalid `#0`).
  - **Streamed-sound guard (`fixStreamedSounds`) and the intro music's early decode (`earlyMusic`):** see "Streamed sounds on HTML5" under Gotchas.
  - PWA links: manifest (`src/web/pwa/`; `display: fullscreen`, `display_override` `[fullscreen, standalone]`, no orientation lock, `start_url`/`scope` `../` relative to `html5game/` so it works under a subpath), `apple-touch-icon`, `theme-color`, `mobile-web-app-capable`. **No `apple-mobile-web-app-status-bar-style`:** `black-translucent` put the page under the status bar, and since iOS 26 the home-screen app blurs that band and a bit below it (the top of the picture in portrait; no CSS turns it off). With the default opaque status bar `env(safe-area-inset-top)` is 0 in the installed app. iOS caches this with the home-screen icon, so an installed app needs removing and re-adding. Headless Chromium reported no installability errors.
  - **Offline play** (`src/offline.ts`): `registerOffline()` runs from `markReady()` (so registration never shares the first visit's bandwidth) and registers `sw.js` from the site root, unless `window.__fuzz` is set or the URL has `?nosw`. It asks the worker to check for a new build then, when the tab comes back and when the machine comes back online, at most once a minute, and `offlineLine()` turns what it says into `#offline`, a `.hint` line in `#face` under the Start word: `v1.0.0 · Saving for offline play 42%`, then `v1.0.0 · Ready to play offline`. **Only an installed app downloads on its own** (`isApp()`: `display-mode` standalone, fullscreen or minimal-ui, or `navigator.standalone`; the install prompt uses the same test). A browser tab's checks only look (`download: false`), and `#offline-go`, a grey underlined link at the end of the `#offline` line, offers what there is (`v1.4.0 · Save for offline play`, `· Finish saving for offline play`, or `· Ready to play offline · Update to v1.4.1`) and turns into the progress when clicked. `saveOffline()` (`barkley.offlineSave()`; the link, or Enter/Space on it; it is the first thing Tab reaches from Start) downloads, and asks for persistent storage. The version on its own comes from `loadVersion()`, which reads the build's `version.json` and runs whether or not the worker does (everything but the fuzz harness), so a `?nosw` page and a browser without service workers still show `v1.0.0`; anything the worker has said wins. The Start screen is removed on Start and the download carries on without it. `src/README.md` ("Offline play") has the worker's design; the short of it: one cache per build, a new build downloaded in full (reusing by hash every file it didn't change) before the old one is deleted, a page pinned to the build it loaded with.
...
- **`import.mjs`**: after importing, it writes `options/html5/options_html5.yy` (the importer never creates it) and copies `src/web/index.html` beside it. It adds extensions with `addExtension(name, file, functions)`: it writes `extensions/<name>/<name>.yy`, the stub beside it, and adds it to the `.yyp`. `Fullscreen` always; `Resume` when the GMX has `scripts/resume_save.gml`, `Saves` when it has `scripts/sSaveData.gml`, `Touch` when `key_doset.gml` calls `touch_keys`, `Gamepad` and `Controls` when it calls `pad_keys` (nothing in the GML calls `controls_show`; the page does), `Crash` when `resume_tick.gml` calls `crash_put`, so faithful imports get only Fullscreen. The `.yy` field set is copied from a current-format JS extension; Igor rejects missing fields. JS functions are `kind` 5; argument and return types are 1 (string) or 2 (real). Obfuscation keeps extension function names. Igor fills in the template's `${GM_HTML5_*}` placeholders. The Included Files are the manifest and `icon-180/192/512.png`, resized with ffmpeg from `src/web/pwa/icon.png`.
...
- **Igor:**
  ...
  - Sizes: the minified deploy bundle is ~6.9 MB (~1.0 MB gzipped); the whole `out/` dir is ~156 MB straight from Igor, almost all sound, and ~100 MB once `writeBuild` has deleted the `.ogg` copies (Igor writes every sound as both `.ogg` and `.mp3`; the page reports no Ogg support, so only the MP3s are ever loaded, and a sound with no MP3 stops the build).
- **The page app is not obfuscated or minified** (`app/barkley.js`, readable even in an obfuscated build). Reading it is the quickest way to tell a stale page from a real bug. The extension files beside `BarkleyLTS.js` (`<prefix>_<name>.js`; a new extension shifts every prefix) are one-line stubs.
...
- **Streamed sounds on HTML5** (all the music is streamed, `compression` 3): the runtime downloads and decodes a track on every play, then starts the sound object that asked even if the game stopped it during the decode, and by then it may have reused that object for the next sound; the late sound is untracked (`audio_is_playing` never sees it) and can't be stopped. A stop during the download does cancel (it aborts the XHR). The page's `fixStreamedSounds` (`runtime/audio.ts`) wraps the runtime's sound class (found by its `play`/`stop`/`start`/`pause` methods) with a generation count, tags the download a play starts, and its `decodeAudioData` wrapper never returns a buffer whose sound moved on. The intro's `mSpace` took 3-7 s from play to sound, hence `earlyMusic`. To trace audio, inject a logger into a test build's `<head>` that wraps `AudioBufferSourceNode.prototype.start/stop`, `decodeAudioData` and `XMLHttpRequest.prototype.open`, and wrap `gml_Script_sA` and `audio_*` in a `js:` step.
- **Rendering on HTML5:** the runtime ignores `devicePixelRatio`, and `window_set_size` writes only the canvas backing store, never `canvas.style` (the runtime does rewrite `canvas.style.cssText` in its fullscreen path). The touch extension pins the CSS size back to the window in a rAF loop (`touch_pin`) while `oScreenFill` sizes the backing store to `browser_* x touch_dpr()`, which gives real device pixels. Neither mode remaps `mouse_x`/`mouse_y`, but only unreachable dev leftovers read the mouse.
...
- **v1.4.1 (`src/version.json`; not deployed yet): the page around the game is a SolidJS app (2026-09-25).** `src/web` became a Vite +
  SolidJS + TypeScript project (see "The page" under Architecture): every piece of UI is a component, the runtime glue
  and the seven extensions' code are typed modules, and the extensions' files are stubs. What a player sees is meant to
  be unchanged, and four things differ on purpose: closing the Controls panel puts focus back on Start; the install
  prompt, a panel like the others now, holds focus and keeps keys pressed in it from the game; a malformed
  save code in the Saves panel's Import now says "Could not import: …" (it threw out of the click handler, and the
  crash recorder took that for the game crashing); and a crash report's `bundle` names the game's script (it named
  whichever extension script the runtime had added to the head first). The touch overlay's fades now really fade
  (it rebuilt its SVG on every change, so they never ran). Checked on a stand-in runtime only (a fake `BarkleyLTS.js`
  that loads the extension files, calls `barkley_loading`, logs "Entering main loop...", calls the GML's functions each
  frame and draws a rectangle), old page against new in headless Chromium: the Start screen, the resume footer, the
  Controls panel and its key and fake-pad test, the Saves panel and its import, a BUG report (its JSON identical in
  keys and events) and a crash, and on a 390x844@3 phone the install prompt, the touch overlay with A held, the stick,
  the sheet, the D-pad, the SET KEYS note and landscape all came out **pixel-identical** (the only differing pixels
  ...
  built it from the exe (still numbered v1.4.0) with the stubs (ProjectTool and Igor took them; the runtime logged seven
...
- **v1.4.0 is live on `https://wyatt-stanke.github.io/barkley/`** (CI run `36085824557`, on `178d974`, the merge of
  ...
  stand-in build (the real `index.html`, `sw.js` and `version.json` from `offline.mjs`, a stub `GameMaker_Init`) in
  headless Chromium, at 1024x768 and 390x844: a first visit in a tab caches nothing and shows the link; a click on
  it saves the copy with the progress in its place, and a click on the words still starts the game; after a deploy the tab keeps playing its copy and offers the update,
  which Enter on the focused link takes without starting the game; with the server gone the copy loads with no
  link; an installed app (`navigator.standalone`) downloads with no link; a `check` message with no `download`
  (every page before this) still downloads; a download that fails after the start-up files offers "Finish saving",
...
- **Not verified yet:**
  ...
  - On a real phone: PWA install, full-screen launch, the install prompt, the iOS separate-storage note, touch controls, the safe-area layout and the landscape joystick zone beside the Dynamic Island, and that the portrait blur is gone after re-adding the app. The app icon is `src/web/pwa/icon.png`; `import.mjs` resizes it at import time, so only a fresh import ships a new one. Keep icon content inside the centre 80% for the maskable crop.
...
- **Play-test recipes:**
  ...
  - Offline play: `SW=1 node src/playtest.mjs …` leaves the service worker in. The play-test is a browser tab, so
    nothing downloads until a `js:` step calls `barkley.offlineSave()` (the link itself is gone once Start is
    clicked, the function isn't); then a `js:` step reading
