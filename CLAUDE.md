...
node src/fuzz.mjs build build/outputs/barkley-1.1.1/BarkleyLTS.yyp build/fuzz/build
...
- `build/outputs/`: migrations and their imports. **The user wants only the latest kept**: when you make a new one, delete the superseded migration and import (the user OK'd that).
  - Latest: `barkley-1.1.1.gmx` (plus `.code`) and its import `barkley-1.1.1/BarkleyLTS.yyp` (2026-09-20): audit clean, importer converted all GML, all six extensions (Gamepad is the new one) plus the PWA files. This is what is deployed. The offline layer needs no re-import: `fuzz.mjs build` writes `sw.js` and `version.json` into the build.
...
**Live: `068a661`** (2026-09-20), v1.1.1, the modernized build of `barkley-1.1.1`, deployed with `deploy.mjs`: game controllers (`modernized/11`). Build id `fc67bc3fa96658d5`, 261 files cached. `BarkleyLTS.js` md5 `91b616bed8cc651db3627978a1948115`; `index.html` md5 `251e3b4cca50dbaa0fc8d639181e5a90`; `sw.js` md5 `8352242ca58c04a4c488f81a8d89eef7`; `version.json` md5 `293001f888bf21f4cef68f6dfb63daf0`; shims `tph_crash.js`, `uph_gamepad.js`, `vph_touch.js`, `wph_saves.js`, `xph_resume.js`, `yph_fullscreen.js`, plus `inter.woff2` and `inter-OFL.txt` (a new extension shifts every shim's prefix, so they must ship with their bundle). Earlier deploys: `58167a6` (v1.1.0, offline play, the battle transition, the dialog key fix, the new icon), `46e21c9` (patch 19, battles drawing blank), `db2beaf` (a rebuild of the same v29 code, no change), `3728aa7` (the iOS touch fixes and the streamed-music guard), `224a51a` (the Start screen's option not to resume), `479ed0f` (v32, redesigned page, loading behind Start), `36ef5ee` (v28, installable app), `9e360b0` (v27, touch controls), `be2ba4e` (v26, volume, Saves panel), `83e6d2b` (v25, first minified unobfuscated deploy), and older ones back to v12.
